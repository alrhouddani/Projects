...
# - Create an ad account: run To run: .\Create_or_Remove_Account.ps1 -FirstName "firstname of the user" -LastName "LastName of the user" -option  create -type 
...
param(
    ...
    [parameter(mandatory)] [string] $option = $null,
    [Parameter(ParameterSetName = 'remove')][string] $notinuse = $null #not in use

   
)
# type swtich using if sta
switch ($type) {
